    ...
    return (
        <div className='flex justify-between p-4 lg:p-0 h-24'>
            ...
            {/* absolute inset-0 lg:inset-auto h-24 lg:h-auto */}
                <div className={`flex items-center xl:p-6 lg:static ${searchInputWraper}`}>
                    <div onClick={isClicked2} className={`rounded-l-md lg:pl-6   px-6 lg:pr-2 h-14 flex items-center text-3xl lg:bg-white lg:bg-opacity-10 ${openStyle2}`}>
                        <BiSearch />
                    </div>
                    <div onClick={isClicked2} className={`rounded-l-md lg:pl-6   px-6 lg:pr-2 h-14 flex items-center text-3xl lg:bg-white lg:bg-opacity-10 ${closeStyle2}`}>
                        <BiLeftArrowAlt />
                    </div>
                    {/* w-[85vw] */}
                    <input type='text' className={`rounded-r-md text-lg outline-none pl-2 lg:pl-0 lg:w-[32rem] bg-transparent lg:bg-white lg:bg-opacity-10 lg:px-4 h-14 ${searchInput}`} placeholder='Search items,collections, and accounts...' />
                </div>
                <div className={`flex lg:ml-4 flex-col duration-300 lg:flex-row absolute lg:static inset-0 lg:inset-auto	bg-[#04111d] lg:bg-transparent bg-opacity-95 z-50 mt-24 lg:mt-0 font-semibold overflow-hidden ${openContentStyle}`}>
